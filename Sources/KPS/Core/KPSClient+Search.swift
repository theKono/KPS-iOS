...
extension KPSClient {
    ...
    public func vectorSearch(with keyword: String, completion: @escaping(Result<KPSSearch, MoyaError>) -> ()) {
        ...
        request(target: .vectorSearch(keyword: keyword, server: KPSClient.config.baseServer), completion: resultClosure)
    }
}
